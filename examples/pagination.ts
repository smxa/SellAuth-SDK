...
async function main() {
  ...
  if (!token || !shopId) { console.error('Set SELLAUTH_TOKEN and SELLAUTH_SHOP_ID'); process.exit(1); }
  const client = new SellAuthClient({ apiKey: token });
  const productsApi = client.products(shopId);

  let count = 0;
  for await (const p of paginateAll(({ page, perPage }) => (
    productsApi.list({ page, perPage }).then(arr => ({ data: arr }))
